...
bool comp2 (toado p, toado q)
{
    ...
    if (cw == 0) return (comp(p,q)); // dễ sai
    return ( cw> 0);
}
...
{
    sort (a+1, a+n+1, comp); // lấy phần tử thấp nhất
    sort (a+2, a+n+1, comp2); // sort theo góc quay
    b[m=1] = a[1];
    for (int i = 2; i <= n; i++){
...
int main()
{
    ...
    printf("%0.3f\n", chuvi);
    printf("%lld.%s", dientich/2, ((dientich % 2) ? "500" : "000"));
